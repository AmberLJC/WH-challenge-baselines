...
def make_graph(csvfile, num_sample):
    edges = pd.read_csv(csvfile)

    edges = edges.loc[(edges['pid1'] < num_sample )&(  edges['pid2'] < num_sample )]
...
def get_subgraph_inf_neighbor_data(subgraph, ego_pid, si_table, delta, cutoff, past_window):
    ...
    for day in valid_days:
        ...
        # print(f"Process day {day}")
        min_day = max(0, day - past_window)

        back_subgraph = si_subgraph[
            si_subgraph.index.get_level_values("infected").to_series().between(min_day, day).values]
        ...
        state_t_d = si_subgraph[si_subgraph.index.get_level_values("infected").to_series().between(day, day + delta,
                                                                                                   inclusive="right").values]
        # end = time.time()
        # print(f'Build subgraph using {end - start}s')

        outputs[i, 0] = subgraph_t
        # outputs[i, 1] = neighbor_t
        outputs[i, 1:3] = comp_node_buckets(subgraph, ego_pid, back_subgraph, [1, 2])
        # start = time.time()
        # print(f'Discover neighbor using {start-end}s')
        ...
        i += 1
        # end = time.time()
        # print(f'Calculate infected weight using {end - start}s')
...
def make_data(pid, graph, disease, pop, past_window, cutoff, delta=7):
    ...
    # end = time.time()
    # print(f'Processing subgraph for person {pid} using {end - start}s')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-date", default=0, type=int,
                        help="Min date to generate data from (used for making evaluation set)")
    parser.add_argument("--is-eval", action="store_true", default=False,
                        help="exclude positive instances (assume input file is training)")
    ...
    parser.add_argument("--num_sample", type=int, default=1000000, help="total number of jobs")

    args = parser.parse_args()
    begin = time.time()

    print("Start data preprocessing")
    pop = pd.read_csv(args.pop_file, engine="pyarrow")


    disease_data = pd.read_csv(args.disease_file, engine="pyarrow")
    disease_data = disease_data.loc[disease_data['pid'] < args.num_sample]

    si_table = make_si_table(disease_data)
    min_client_id = pop['pid'][0]
    pop = pop.loc[pop['pid'] < min_client_id + args.num_sample]
    pop.set_index("pid", inplace=True)

    print("Finish reading input data")
    graph = make_graph(args.graph_file, args.num_sample)
    data = []
    ...
    # vertex_names = [*range(len(graph.vs) )]
    ...
    for pid in pid_part:
        ...
        if pid % 1000 == 0:
            print(f"Progress of {args.pid_partition}: {pid}/{len(pid_part)}")

    end = time.time()
    print(f'Processing partition {pid} using {end - begin}s')
