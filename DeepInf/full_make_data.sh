...
do
    TRAIN_DIRNAME=../full_data/gnn/train/data_$PART
    RWR_DIR=../full_data/random_walks/part_$PART

    # Make training data

    mkdir -p $TRAIN_DIRNAME
    python ./data/extract_sample.py $RWR_DIR $TRAIN_DIRNAME --sample_size 300000 --disease-file $DISEASE --pop-file $POP 
    ...
    TEST_DIRNAME=../full_data/gnn/eval/data_$PART
    mkdir -p $TEST_DIRNAME 

    python ./data/extract_sample.py $RWR_DIR $TEST_DIRNAME --sample_size 300000 --test-set --disease-file $DISEASE --test-date-start 50 --pop-file $POP
...
FULL_TRAIN_DIR=../full_data/gnn/train/full
FULL_EVAL_DIR=../full_data/gnn/eval/full

mkdir -p $FULL_TRAIN_DIR
python ./data/collate.py ../full_data/gnn/train $FULL_TRAIN_DIR

mkdir -p $FULL_EVAL_DIR
python ./data/collate.py ../full_data/gnn/eval $FULL_EVAL_DIR
